...
export default function VideoSub_confirm() {
  ...
  const subData = useSelector((state) => state.cart.videosub_order)
  //點選按鈕清除購物車redux
        ...
        <div className={`collapse ${styles} row`} id="collapseOrderdetail">
          ...
          >
            ...
            >
              ...
              <div className={`${styles.productItem} col-3`}>
                {subData.formattedCurrentDate}
              </div>
              <div className={`${styles.productItem} col-3`}>
                {subData.formattedFutureDate}
              </div>
              <div className={`${styles.productItem} col-3`}>
                NT${subData.price}
              </div>
              ...
              <div className={`col-6`} style={{ height: '100%' }}>
                ...
                >
                  ...
                  <div className={`${styles.orderdetailRight} col-6`}>
                    <div>{subData.shortId}</div>
                    <div>{username}</div>
                    <div>信用卡付款</div>
                    <div>NT ${subData.price}</div>
                    <div>{subData.formattedCurrentDate}</div>
                  </div>
